(ns taoensso.sente.server-adapters.generic-node-ws
...
(defn- make-chan [websocket response-open? response]
  (let [write #(try
                 (.write %1 %2)
                 (catch :default e))
        send #(try
    ...
    (if websocket
      (reify i/IAsyncNetworkChannel
        (send!* [this msg close-after-send?]
          ...
          (let [pre-open? (= (.-readyState websocket) (.-OPEN websocket))]
            (send websocket msg)
...
(def sente-web-server-adapter
  (reify
    ...
    (ring-req->net-ch-resp
      ...
      (let [{:keys [on-open on-close on-msg]} callbacks-map
            {:keys [websocket response body]} ring-req
            response-open? (atom true)
            chan (make-chan websocket response-open? response)]
        (on-open chan)
        (if websocket
          (do
            (.on websocket
